...
Hay una diferencia entre el estimador de @kde-variedad y lo que #kdc y #fkdc computan: el factor $1 slash theta_(X_i) (p)$ se omite, es decir, se toma $theta equiv 1$. Es una decisión forzada --- sin conocer #MM no hay forma de evaluar $theta$, como se discutió en @vol-dens y sus observaciones --- y conviene decir qué cuesta. Cerca de $p$, la densidad de volumen admite el desarrollo clásico en términos de la curvatura de Ricci #footnote[cf. @besseManifoldsAllWhose1978[§6.3]; la expansión se obtiene de los campos de Jacobi a lo largo de las geodésicas que irradian de $p$.], $theta_p (exp_p v) = 1 - 1/6 op("Ric")_p (v, v) + O(norm(v)^3)$, de modo que dentro de una ventana de radio $h$ el factor omitido vale $1 + O(h^2)$: del mismo orden que el término de sesgo $h^4$ --- al cuadrado --- en la cota de @pelletierKernelDensityEstimation2005, con lo que cambia las constantes pero no la tasa de convergencia. Para clasificación el daño es todavía menor, porque el mismo factor afecta a todas las clases que comparten la variedad alrededor de $p$ y se cancela en buena medida en el cociente de la regla de Bayes. Nada de esto lo convierte en inocuo: en variedades de curvatura alta relativa a $h$ el supuesto falla, y es una de las debilidades que retomamos en @trabajo-futuro.
...
La tercera es la densidad de volumen y, con ella, la consistencia de #fkdc. Tomamos $theta equiv 1$ (@omision-theta) y argumentamos que el error es de segundo orden en $h$; falta comprobarlo. La esfera $S^2$ ofrece el experimento más barato, porque allí $theta_p (q)$ tiene la forma cerrada de @henryKernelDensityEstimation2009 reproducida en @vol-dens: basta muestrear clases sobre la esfera y entrenar #kdc con y sin el factor para medir cuánto cambia el $R^2$ en función de $h$. El paso siguiente es estimar el término de curvatura a partir de la muestra --- los residuos del análisis de componentes principales local de @vincentManifoldParzenWindows2002 son un candidato natural --- y corregir $theta$ en lugar de ignorarla. Del lado de la teoría, el estimador que de hecho entrenamos no está cubierto por ningún teorema: @pelletierKernelDensityEstimation2005 supone conocida la distancia $dg$, y @groismanNonhomogeneousEuclideanFirstpassage2022 garantiza la convergencia de #sfd a $cal(D)_(f, beta)$, pero no hay un resultado que combine ambos y diga que el núcleo evaluado sobre #sfd estima consistentemente _alguna_ densidad. Conjeturo que la respuesta pasa por notar que $cal(D)_(f, beta)$ es la distancia de la métrica conforme $g_f = f^(-2 beta) g$, y que respecto de esa métrica cambian tanto la densidad que se estima como su densidad de volumen; en particular, $alpha$ y $theta$ no serían independientes, lo que daría un sentido preciso a la observación empírica de que un $alpha > 1$ solo aparece donde la curvatura importa.

Quedan, por último, tres ajustes de diseño experimental que no cambian las preguntas pero sí la calidad de las respuestas: reemplazar el grafo completo por el de $k$ vecinos con el $k = O(2^(d_MM) ln n)$ que garantiza @chuExactComputationManifold2019, para escalar #sfd a muestras diez veces mayores; entrenar los clasificadores blandos directamente con el $R^2$ de McFadden como _score_ y calibrar #svc para poder compararlo por $R^2$; y reemplazar el recuento de podios de @resultados por una comparación estadística sobre los 20 datasets, con el test de Friedman y el post-hoc de Nemenyi que propone @demsarStatisticalComparisonsClassifiers2006.

...
En el extremo opuesto, el dataset de `digitos` ($D = 64$) resultó el caso más favorable para la distancia de Fermat en todo el experimento: #fkdc fue el mejor clasificador global con $R^2 approx 98$, superando a #kdc ($R^2 approx 97$), #gbt ($R^2 approx 94$) y todos los demás. En este espacio de alta dimensión con estructura manifiesta de variedades, la distancia de Fermat captura relaciones que la euclídea no detecta. Sin embargo, en `mnist` ($D = 96$ tras reducción por PCA) la relación se invierte para el par de densidad --- #kdc supera a #fkdc --- lo cual sugiere que la estimación de #sfd puede degradarse en dimensiones muy altas.
...
En los casos de peor rendimiento ---particularmente los datasets de 15 dimensiones con ruido añadido, así como `helices_12` y `hueveras_12`--- la degradación se explica por la combinación de ruido excesivo para el tamaño muestral disponible, o por no haber realizado una reducción _lineal_ de dimensionalidad antes de aplicar la distancia de Fermat. Las 12 dimensiones de ruido añadidas a los datasets tridimensionales degradaron severamente a todos los estimadores por densidad de núcleos, cuya localidad los hace particularmente vulnerables a la maldición de la dimensionalidad. Cabe notar, sin embargo, que _todo_ algoritmo evaluado ---incluso #logr sin escalar--- obtuvo el mejor rendimiento, o uno muy cercano, en al menos un dataset. No existe un único algoritmo universalmente óptimo; la elección sigue quedando a criterio del investigador según las características de cada problema. La distancia de Fermat y nuestras implementaciones de #fkdc y #fkn constituyen una herramienta más en la caja del investigador.

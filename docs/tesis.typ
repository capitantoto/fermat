...
Esperábamos alguna ventaja más notable de #fkdc sobre #kdc, o de #fkn sobre #kn, que no se comprobó, y los hiperparámetros elegidos repiten el patrón de `lunas_lo`. La regla de parsimonia eligió para #fkdc $alpha = 1$ en las #reps semillas, con una ventana algo menor que la de #kdc ($h approx 5.6$ contra $7.4$) que la grilla de #kdc no contenía: de allí la leve ventaja. Para #fkn, el _score_ de validación cruzada se maximizó con $alpha$ entre $1.75$ y $3.75$ en _todas_ las semillas, pero la mejora rara vez superó el desvío estándar entre pliegos y la parsimonia devolvió $alpha = 1$ en 13 de las 25, casi siempre con $k$ entre 22 y 41 vecinos: una mejora consistente pero pequeña para el clasificador de vecinos, y ninguna para el de densidad.

==== `mnist`

A `mnist` ($N = 800$, $K = 10$) se lo redujo de $d = 784$ a $d = 96$ dimensiones por PCA #footnote[número que se eligió para conservar al menos el 90% de la variación en los datos originales] y de un $N=60000$ original se elgió una muestra al azar de para volverlo manejable #footnote[es decir, que el]. #kdc ($R^2 approx 0.77$) supera a #fkdc ($approx 0.74$) con menor dispersión; ambos superan a #gbt y quedan a la par de #logr en exactitud y $R^2$, con #svc como el más exacto. 
...
Aquí ni siquiera el maximizador del _score_ de validación cruzada se aparta de $alpha = 1$ para #fkdc, en ninguna semilla; para #fkn se repite lo de `digitos`, con $alpha = 1$ bajo parsimonia en 18 de las 25. Lo que sí merece atención son los anchos de banda seleccionados, entre $316$ y $562$ para #kdc y #fkdc: parecen enormes, pero hay que leerlos contra la escala de los datos, que la @tabla-escala-distancias resume para los cinco datasets orgánicos. En `mnist` la distancia mediana de una observación a su vecina más cercana es $approx 1300$, y la distancia mediana entre dos observaciones cualesquiera, $approx 2500$. Es la maldición de la dimensionalidad de @kde-mv en acto: mientras que en `iris` o `vino` la distancia típica es entre diez y treinta veces la distancia al vecino más cercano, en `digitos` es menos de tres veces y en `mnist` menos de dos. Con las distancias así concentradas no existe una escala en la que el núcleo pese "un vecindario" sin pesar a casi toda la muestra, y la validación cruzada responde encogiendo $h$ muy por debajo de la distancia al vecino más cercano --- $5.6$ contra $18$ en `digitos`, $316$ contra $1300$ en `mnist` ---, hasta que incluso ese vecino recibe un peso casi nulo y cada predicción descansa en una o dos observaciones. El clasificador de densidad degenera así en una versión blanda del de un vecino, lo que explica que #kdc y #kn rindan casi igual en ambos datasets y que $alpha$ no tenga sobre qué actuar.
...
#tabla_csv(
  ...
  raw-cols: ("dataset",),
  caption: [Escala de las distancias euclídeas en los datasets orgánicos (muestra de la semilla de graficación): mediana de la distancia de cada observación a su vecina más cercana, mediana de la distancia entre pares de observaciones y cociente entre ambas.],
  short-caption: [Escala de las distancias en los datasets orgánicos],
) <tabla-escala-distancias>

= Conclusiones

Implementamos la distancia de Fermat muestral como una métrica compatible con los clasificadores de `scikit-learn` --- con estimación _out-of-sample_ y rutinas basadas en primitivos de `numpy` y `scipy` --- y sobre ella dos clasificadores, #fkdc y #fkn, que comparamos con sus pares euclídeos y con cuatro alternativas de referencia en 20 datasets y #reps repeticiones cada uno. Por $R^2$ mediano, #fkdc obtuvo el máximo en 7 datasets y #fkn en 5 --- dos de ellos empatado con #kn ---; #kdc, la implementación del clasificador de @loubesKernelbasedClassifierRiemannian2008, en otros 2. Por exactitud la ventaja se diluye, y #svc resulta casi imbatible.
...
La tercera es la densidad de volumen y, con ella, la consistencia de #fkdc. Tomamos $theta equiv 1$ (@omision-theta) con un argumento cualitativo; falta ponerlo a prueba. La esfera $S^2$ ofrece el experimento más barato, porque allí $theta_p (q)$ tiene la forma cerrada de @henryKernelDensityEstimation2009 reproducida en @vol-dens: basta muestrear clases sobre la esfera y entrenar #kdc con y sin el factor para medir cuánto cambia el $R^2$ en función de $h$, y a partir de qué curvatura relativa a $h$ el supuesto deja de ser razonable. El paso siguiente sería estimar $theta$ en lugar de ignorarla, retomando el hilo de @omision-theta: la formulación de @besseManifoldsAllWhose1978[§6.3] en términos de campos de Jacobi describe cómo se abren las geodésicas alrededor de $p$, y el giro del espacio tangente estimado por componentes principales locales entre observaciones vecinas es una medida muestral de lo mismo; si esa correspondencia se puede hacer precisa, daría una corrección de $theta$ computable con lo que #fkdc ya calcula. Del lado de la teoría, el estimador que de hecho entrenamos no está cubierto por ningún teorema: @pelletierKernelDensityEstimation2005 supone conocida la distancia $dg$, y @groismanNonhomogeneousEuclideanFirstpassage2022 garantiza la convergencia de #sfd a $cal(D)_(f, beta)$, pero no hay un resultado que combine ambos y diga que el núcleo evaluado sobre #sfd estima consistentemente _alguna_ densidad. Sospecho que la respuesta pasa por observar que la distancia de Fermat es la distancia de una métrica que depende de la propia densidad, y que respecto de esa métrica cambian tanto la densidad que se estima como su densidad de volumen; si así fuera, $alpha$ y $theta$ no serían independientes, lo que daría un sentido preciso a la observación empírica de que un $alpha > 1$ solo aparece donde la curvatura importa.

Quedan, por último, tres ajustes de diseño experimental que no cambian las preguntas pero sí la calidad de las respuestas: reemplazar el grafo completo por el de $k$ vecinos con el $k = O(2^(d_MM) ln n)$ que garantiza @chuExactComputationManifold2019, para escalar #sfd a muestras diez veces mayores; entrenar los clasificadores blandos directamente con el $R^2$ de McFadden como _score_ y calibrar #svc para poder compararlo por $R^2$; y reemplazar el recuento de podios de @resultados por una comparación estadística por rangos sobre los 20 datasets, que dé cuenta de los empates y de la variabilidad entre semillas.

...
En altas dimensiones, la distancia natural al espacio donde se registran las observaciones de un elemento aleatorio --- típicamente la distancia euclídea en $RR^d$ --- no distingue adecuadamente entre elementos en la vecindad local de una observación y otros más alejados, lo cual dificultaría enormemente cualquier tarea de clasificación basada en distancias. Según la "hipótesis de la variedad", los elementos aleatorios bajo estudio podrían posarse sobre una variedad de dimensión intrínseca potencialmente mucho menor a la ambiente --- la del soporte de registro ---, y en tal contexto sería posible aprender una distancia de los datos que conserve utilidad en alta dimensión. Habierndo analizado la ggenealogía y los fundamentos teóricos del aprendizaje de distnacia, proponemos un clasificador basado en densidad por núcleos para elementos aleatorios definidos en variedades riemannianas compactas sin frontera y a priori desconocida, utilizando la distancia de Fermat aprendida de los datos disponibles. El estudio sistemático de estas construcciones revela que los clasificadores basados en densidad son competitivos contra métodos del estado del arte, sin que ninguno domine con claridad. En datasets de alta curvatura y poca separación entre clases, clasificadores entrenados con distancia de Fermat aportan mejoras en términos de $R^2$ sobre sus pares con distancia euclídea, sin perder en exactitud. En escenarios bien muestreados, donde la ventana del clasificador es pequeña y define un vecindario por dentro del radio de inyectividad de la muestra, la distancia de Fermat no se diferencia de la euclídea y el beneficio es casi nulo.
#v(0.5em)
...
== La densidad de volumen, omitida <omision-theta>

Hay una diferencia entre el estimador de @kde-variedad y lo que #kdc y #fkdc computan: el factor $1 slash theta_(X_i) (p)$ se omite, es decir, se toma $theta equiv 1$. Es una decisión forzada --- sin conocer #MM no hay forma de evaluar $theta$, como se discutió en @vol-dens y sus observaciones --- y conviene decir qué supone. Lo único que sabemos de $theta$ es lo que allí se dijo: vale exactamente $1$ en variedades planas, vale $1$ en el propio punto $p$ y tiende a $1$ a medida que $q$ se acerca a $p$. Como el núcleo solo pesa observaciones a distancia comparable a $h$, y @pelletierKernelDensityEstimation2005 exige de todos modos $h < "iny" MM$, el factor omitido es cercano a $1$ justamente en la región que importa, y tanto más cuanto menor sea $h$ respecto de la curvatura de #MM. Para clasificación el efecto es todavía menor, porque el mismo factor afecta a todas las clases alrededor de $p$ y en buena medida se cancela en el cociente de la regla de Bayes. Nada de esto lo convierte en inocuo: es un supuesto que no verificamos, que falla en variedades muy curvas respecto de $h$, y que retomamos como debilidad en @trabajo-futuro. No es, sin embargo, un callejón sin salida: @besseManifoldsAllWhose1978[§6.3] expresa $theta$ mediante campos de Jacobi a lo largo de las geodésicas que irradian de $p$ --- es decir, en función de cómo se separan o se juntan las geodésicas vecinas ---, y esa información local es del mismo tipo que la que captura el análisis de componentes principales alrededor de cada observación de @vincentManifoldParzenWindows2002: cómo cambia el espacio tangente estimado de una observación a sus vecinas.

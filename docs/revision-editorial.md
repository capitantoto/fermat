...
4. **Corridas `_std` completas.** Las 2025 tareas terminaron y todas las tablas y fichas salen de ellas. Resumen: `pinguinos` f-KDC 0.42 → 0.96 (a 0.005 de s-LR); `vino` familia K 0.43 → 0.84–0.88 (GBT y s-LR 0.90 no se mueven); `pionono_12` y `eslabones_12` K sube a 0.30 / 0.41, GBT sigue en 0.79 / 0.92; `helices_12` y `hueveras_12` sin cambio; `digitos` K *baja* 0.98 → 0.87; `mnist` K se desploma 0.77 → 0.40 (whitening de las componentes principales). Texto de 4.4 y Conclusiones ya alineado con estos números.

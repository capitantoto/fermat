...
4. **Tabla de `pinguinos` crudo vs. estandarizado** (§4.4.1) usa por ahora un CSV provisorio de la corrida one-off. `make viz` lo reemplaza cuando terminen las 225 tareas de `pinguinos_std`; releer la frase «f-KDC registra el mejor R² del dataset» contra los números finales.
